...
class EyeGazeNetV3(nn.Module):
    def __init__(self):
        ...
        self.block2 = self._make_res_block(64, 64)
...
def train_model(config):
    ...
    os.makedirs("training_results_6", exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = f"training_results_6/{timestamp}"
    ...
    for epoch in range(config.epochs):
        ...
        if val_loss < best_loss:
            ...
            traced_model.save(f"{results_dir}/model_6v2.pt")
...
if __name__ == "__main__":
    config = Config()
    print_model_summary(EyeGazeNetV3())
