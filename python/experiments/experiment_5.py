...
class EyeGazeNetV2(nn.Module):
    def __init__(self):
        ...
        self.attention = nn.Sequential(
            nn.Conv2d(256, 1, 1),  # Spatial attention map
            nn.Sigmoid()
        )
...
def train_model(config):
    ...
    os.makedirs("training_results_5", exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = f"training_results_5/{timestamp}"
    ...
    for epoch in range(config.epochs):
        ...
        if val_loss < best_loss:
            ...
            traced_model.save(f"{results_dir}/model_5.pt")
...
if __name__ == "__main__":
    config = Config()
    print_model_summary(EyeGazeNetV2())
