...
class EnhancedEyeGazeNet(nn.Module):
    def __init__(self, screen_width=1440, screen_height=900):
        ...
        # Правильная инициализация последнего слоя
...
def train_model(config):
    ...
    os.makedirs("training_results_2", exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = f"training_results_2/{timestamp}"
    ...
    for epoch in range(config.epochs):
        ...
        if val_loss < best_loss:
            ...
            traced_model.save(f"{results_dir}/model_2.pt")
